	...
	Physics.body('player', 'circle', function (parent) {
		...
		return {
			...
			shoot : function () {
				var self = this;
				var world = this._world;
				if (!world || self.delayShoot) {
					//Don't 'shoot' if shot fired within 600ms
					return self;
				}
				var angle = this.state.angular.pos;
				...
				var laser = Physics.body('circle', {
						...
						vx : (0.5)*cos + this.state.vel.get(0),
						vy : (0.5)*sin + this.state.vel.get(1),
						radius : 5,
						mass : 1,
						styles : {
							fillStyle : '#0066FF'
				...
				// limit 'shoot' to every 600ms
				self.delayShoot = true;
				setTimeout(function () {
					self.delayShoot = false;
				}, 600);
